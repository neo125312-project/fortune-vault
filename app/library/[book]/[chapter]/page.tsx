...
export async function generateMetadata({ params }: { params: Promise<{ book: string; chapter: string }> }) {
  ...
  if (!result) return {};
  return {
    title: `${result.chapter.title} · 《${result.book.title}》· 紫微斗數古籍`,
            ...
            >
              ...
              {p.translation && (
                <div style={{
                  ...
                  <span style={{ fontSize: '10px', color: 'var(--ac)', marginRight: '6px' }}>白話</span>
                  {p.translation}
                </div>
              )}
              {p.niNote && (
                <div style={{
                  ...
                  <span style={{ fontSize: '10px', color: 'var(--ji)', marginRight: '6px' }}>倪師注</span>
                  {p.niNote}
                </div>
              )}
            </div>
